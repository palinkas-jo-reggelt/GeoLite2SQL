...
.FUNCTIONALITY
	...
	4) Compares database to new data for incremental changes
...
.NOTES
	...
	* Initial loading of the database takes over one hour - subsequent updates are incremental, so they take less time
...
$RunLog = "$PSScriptRoot\RunLog.log"
...
If (Test-Path $RunLog) {Remove-Item -Force -Path $RunLog}
